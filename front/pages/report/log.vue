<template>
  <div class="container">
    <el-collapse v-model="activeNames">
      <el-collapse-item v-for="(log, i) in data" :key="i" :name="i">
        <template slot="title">
          {{ log.name }} | 运行时长：{{ log.duration }} | 结果状态：{{ log.status }}"
        </template>
        <el-timeline>
          <el-timeline-item
            timestamp="初始化"
          ...
          >
            <el-card>
              ...
              <div v-if="log.validator">
                <div v-for="(item, i) in log.validator" :key="i">
                  <el-row>
                    <el-col :span="8">
      ...
      this.$axios({
        ...
        if (res.data.status === 0) {
          ...
          for (const i in this.data) {
            this.data[i].response.data = JSON.parse(this.data[i].response.data)
          }
        } else {
          this.$message({
            type: 'warning',
